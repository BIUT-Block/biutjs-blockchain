...
class SECTokenBlockChain {
  ...
  _generateGenesisBlock () {
    let extraData = 'Hello World'
    if (process.env.secTest) {
      extraData = 'Test Network Genesis Block'
    }
    if (this.chainName === 'SEC') {
      return new SECTokenBlock({
        ...
        Beneficiary: '',
        Difficulty: '',
        ExtraData: 'SEC blockchain ' + extraData,
        Nonce: '',
        Transactions: [{
          TxHash:
      ...
      return new SECTokenBlock({
        ...
        Difficulty: '1',
        ExtraData: 'SEN blockchain ' + extraData,
        Nonce: SECUtils.zeros(8).toString('hex'),
        Transactions: []
      }).getBlock()
    } else {
      throw new Error('Invalid chain name, should be either SEC or SEN')
    }
  }
